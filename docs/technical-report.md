...
## Appendix C: PDF Conversion Procedure

For a submission-grade PDF from this Markdown file, use Pandoc with a table of contents and numbered sections.

1. Basic conversion:

   pandoc docs/technical-report.md -o docs/technical-report.pdf

2. Recommended conversion (better structure for grading):

   pandoc docs/technical-report.md -o docs/technical-report.pdf --toc --number-sections --pdf-engine=xelatex -V geometry:margin=1in

3. If figure paths fail, run from repository root so relative links resolve correctly.

4. If your grader requires 11pt Times-like output, add:
